...
import es.gob.aapp.csvstorage.webservices.administration.AdminWSService;
...
@WebService(serviceName = "AdminWSService", portName = "AdminWSServicePort",
    targetNamespace = "urn:es:gob:aapp:csvstorage:webservices:administration:v1.0",
    endpointInterface = "es.gob.aapp.csvstorage.webservices.administration.AdminWSService")
...
public class AdminWSServiceImpl implements AdminWSService {
  ...
  public Aplicacion altaAplicacion(Aplicacion aplicacion) throws CSVStorageException {
    ...
    try {
      ...
      if (isAdmin()) {
        if (aplicacion != null) {
          ...
          UserEntity userEntity = userManagerService.findByUsuario(user.getUsername());
  ...
  @Override
  @Transactional(rollbackFor = ServiceException.class)
  public Aplicacion bajaAplicacion(String idAplicacion) throws CSVStorageException {
  ...
  @Override
  public List<Aplicacion> listaAplicaciones() throws CSVStorageException {
  ...
  @Override
  public List<InfAdicional> infAdicionalAltaApp() throws CSVStorageException {
    ...
    if (isAdmin()) {
      ...
      listaInfAdicional = new ArrayList<>();
        ...
        if (applicationList != null && applicationList.size() > 0) {
          ...
          if (application.getId() == null) {
            ...
            for (ApplicationEntity appBBDD : applicationList) {
              if (!appBBDD.getId().equals(application.getId()) && !hasError) {
                LOG.error("Error, número de serie repetido. ");
                message = "El número de serie del certificado ya está asociado a otra aplicación.";
              }
            }
  ...
  @Override
  public Aplicacion eliminaAplicacion(String idAplicacion) throws CSVStorageException {
